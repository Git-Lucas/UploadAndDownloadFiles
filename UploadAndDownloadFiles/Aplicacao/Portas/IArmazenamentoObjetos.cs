...
{
    /// <summary>
    /// URL pré-assinada de PUT único, com `Content-Length-Range` igual ao tamanho declarado e o
    /// `Content-Disposition` derivado de <paramref name="nomeOriginal"/> incluído na assinatura.
    /// Retorna também o valor do cabeçalho, que o cliente **deve** repetir literalmente no PUT —
    /// omiti-lo ou alterá-lo invalida a assinatura (`SignatureDoesNotMatch`).
    /// </summary>
    Task<UrlDeUploadUnico> CriarUrlDeUploadUnicoAsync(string chave, long tamanho, string nomeOriginal, CancellationToken cancellationToken = default);

    /// <summary>
    /// Inicia o multipart upload no S3 e retorna o `uploadId`. O `Content-Disposition` é gravado
    /// aqui, no início do upload — as partes não o carregam.
    /// </summary>
    Task<string> IniciarMultipartAsync(string chave, string nomeOriginal, CancellationToken cancellationToken = default);
